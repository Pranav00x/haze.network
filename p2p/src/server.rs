...
use haze_chain::chain::{ChainState, ApplyResult};
use haze_chain::block::Block;
use haze_chain::transaction::{Transaction, TxKernel};
...
impl haze_chain::proposer::BlockBroadcaster for P2pServer {
            ...
            Some(msg) => {
                    match msg {
                        ...
                        P2pMessage::CancelListing { asset_id, seller_pubkey, signature } => {
                            let msg = haze_chain::marketplace::cancel_signing_message(&asset_id, &seller_pubkey);
                        ...
                        P2pMessage::BlocksBatch { blocks, has_more } => {
                            ...
                            for block in &blocks {
                                if aggregate_mode {
                                    ...
                                    if !links || block.header.chain_id != haze_chain::genesis::CHAIN_ID {
