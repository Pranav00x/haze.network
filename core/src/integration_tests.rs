#[cfg(test)]
mod tests {
    use crate::transaction::{Transaction, Input, Output, TxKernel};
    use crate::cut_through::aggregate_and_cut_through;
    ...
    fn empty_registry_root() -> [u8; 32] {
        crate::registry::compute_registry_root(&std::collections::HashMap::new())
    }
    ...
    fn stake_proof(commitment: &Commitment, value: u64, blinding: &Scalar) -> Signature {
        let msg = crate::chain::stake_registration_message(commitment, value);
    ...
    fn apply_block_never_stores_a_block_that_fails_standalone_validation() {
        ...
        let garbage_header = BlockHeader {
            ...
            asset_registry_root: crate::assets::compute_asset_registry_root(&std::collections::HashMap::new()),
            collection_registry_root: crate::collections::compute_collection_registry_root(&std::collections::HashMap::new()),
        };
    ...
    fn test_full_transaction_lifecycle() {
        ...
        let coinbase_value = crate::block::block_reward_at(1) + 10;
        ...
        let reward_commitment = Commitment::new(crate::block::block_reward_at(1), Scalar::zero()).as_point();
    ...
    fn apply_block_accepts_a_non_primary_active_validator_as_fallback_proposer() {
        ...
        let r_coinbase = Scalar::random(&mut rng);
        let reward = crate::block::block_reward_at(1);
        let coinbase_output = Output {
    ...
    // test_dandelion_fluff_timeout and test_p2p_validator_propagation used to
    // live here, but both need haze_p2p, and haze_p2p depends on this crate
    // (haze-chain) - even as a dev-dependency, that produces two separate
    // compilations of haze-chain (one under test, one as haze_p2p's normal
    // dependency) whose types aren't interchangeable. Moved to
    // p2p/src/integration_tests.rs, where haze-chain is just a normal
    ...
    fn test_chain_reorganization() {
        fn create_empty_block(height: u64, prev_hash: [u8; 32]) -> Block {
            ...
            let private_key = Scalar::from(42u64);
            let r_coinbase = Scalar::random(&mut rng);
            let reward = crate::block::block_reward_at(height);
    ...
    fn test_name_registration_applies_and_rolls_back() {
        use crate::registry::{RegisterNameOp, NAME_REGISTRATION_FEE, NameRecord, compute_registry_root};
    ...
    fn block_with_validator_op_registers_and_rolls_back() {
        ...
        let validator_op = crate::chain::RegisterValidatorOp {
    ...
    fn block_with_forged_validator_op_is_rejected_entirely() {
        ...
        let forged_op = crate::chain::RegisterValidatorOp {
            commitment: stake_commitment,
            value: 999_999_999,
            proof: stake_proof(&stake_commitment, stake_value, &r_stake),
        };
    ...
    fn test_name_transfer_applies_and_rolls_back() {
        use crate::registry::{RegisterNameOp, TransferNameOp, NAME_REGISTRATION_FEE, NameRecord, compute_registry_root};
        use bulletproofs::PedersenGens;
    ...
    fn apply_block_rejects_block_containing_wraparound_inflation_attack() {
        ...
        let r_real = Scalar::random(&mut rng);
        let real_value = 1_000_000 + crate::block::block_reward_at(1) + 500_000; // balances against reward + phantom
        let real_output = Output {
        ...
        assert!(!body.validate_with_reward(crate::block::block_reward_at(1)), "sanity: the body alone must already fail with the block reward applied");
    ...
    fn block_reward_at_halves_on_schedule_boundaries() {
        use crate::block::block_reward_at;
        use crate::genesis::{HALVING_INTERVAL_BLOCKS, INITIAL_BLOCK_REWARD};
    ...
    fn genesis_total_minted_matches_sum_of_genesis_outputs() {
        use crate::genesis::{genesis_block, GENESIS_TOTAL_MINTED};
    ...
    fn apply_block_rejects_mismatched_chain_id() {
        ...
        let mut header = BlockHeader {
            ...
            validator_signature: Signature { s: Scalar::zero(), e: Scalar::zero() },
            name_registry_root: empty_registry_root(),
            chain_id: crate::genesis::CHAIN_ID.wrapping_add(1),
            asset_registry_root: crate::assets::compute_asset_registry_root(&std::collections::HashMap::new()),
            collection_registry_root: crate::collections::compute_collection_registry_root(&std::collections::HashMap::new()),
        };
        let msg = header.hash();
    ...
    fn test_asset_mint_applies_and_rolls_back() {
        use crate::assets::{MintAssetOp, AssetRecord, ASSET_MINT_FEE, compute_asset_registry_root};
        use bulletproofs::PedersenGens;
    ...
    fn test_duplicate_asset_mint_in_same_block_is_rejected() {
        use crate::assets::{MintAssetOp, AssetRecord, compute_asset_registry_root};
        use bulletproofs::PedersenGens;
        ...
        let make_op = |rng: &mut OsRng, chain_state: &mut ChainState| -> MintAssetOp {
            let r_in = Scalar::random(rng);
            let input_commitment = Commitment::new(crate::assets::ASSET_MINT_FEE, r_in);
            chain_state.utxos.insert(input_commitment);
            let fee_payment = Transaction {
                inputs: vec![Input { commitment: input_commitment }],
                outputs: vec![],
                kernels: vec![TxKernel {
                    excess: Commitment::new(0, r_in),
                    fee: crate::assets::ASSET_MINT_FEE,
                    signature: Signature::sign(&crate::assets::ASSET_MINT_FEE.to_le_bytes(), &r_in),
                }],
            };
            let owner_secret = Scalar::random(rng);
            let owner_pubkey = Commitment(owner_secret * gens.B_blinding);
    ...
    fn transfer_asset_op_with_unsatisfied_kernel_condition_is_rejected() {
        use crate::assets::{TransferAssetOp, compute_asset_registry_root};
        ...
        registry_after.insert("marketswap-punk".to_string(), crate::assets::AssetRecord {
            asset_id: "marketswap-punk".to_string(),
    ...
    fn rollback_makes_previously_satisfied_condition_unsatisfied_again() {
        use crate::assets::compute_asset_registry_root;
    ...
    fn collection_launch_and_multi_phase_mint_round_trip() {
        use crate::assets::{MintAssetOp, AssetRecord, ASSET_MINT_FEE, compute_asset_registry_root};
        use crate::collections::{LaunchCollectionOp, MintPhase, compute_collection_registry_root, allowlist_leaf};
        use crate::merkle::{merkle_root, build_merkle_proof};
        use bulletproofs::PedersenGens;
        ...
        collections_after_launch.insert(collection_id.clone(), crate::collections::CollectionRecord {
    ...
    fn transfer_of_royalty_bearing_asset_requires_both_payment_kernels() {
        use crate::assets::{MintAssetOp, TransferAssetOp, AssetRecord, ASSET_MINT_FEE, compute_asset_registry_root};
        use crate::collections::{LaunchCollectionOp, CollectionRecord, MintPhase, compute_collection_registry_root};
        use bulletproofs::PedersenGens;
        ...
        let mut header1 = BlockHeader {
            ...
            name_registry_root: empty_registry_root(), chain_id: crate::genesis::CHAIN_ID,
