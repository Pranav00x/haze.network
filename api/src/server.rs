...
impl ApiServer {
    pub async fn start(
        ...
        let publish_allowlist_route = warp::post()
            ...
            .and(warp::body::content_length_limit(MAX_BODY_SIZE))
            .and(warp::body::json())
            .map(|_collection_id: String, _phase_index: u32, entry: haze_chain::allowlist::AllowlistEntry| entry)
...
struct StakeRequest {
    commitment: haze_crypto::pedersen::Commitment,
    value: u64,
...
async fn handle_register_validator(
    ...
    if queued {
        ...
        let msg = haze_p2p::message::P2pMessage::NewValidatorOp(op);
        tokio::spawn(async move {
