...
use haze_chain::compaction::BlockPruneMeta;
...
fn commitment_hex(c: &haze_crypto::pedersen::Commitment) -> String {
    c.to_hex()
}
...
fn all_kernels(block: &Block) -> Vec<&haze_chain::transaction::TxKernel> {
    block.body.kernels.iter()
        .chain(block.name_ops.iter().flat_map(|op| op.fee_payment.kernels.iter()))
        .collect()
}
fn all_inputs(block: &Block) -> Vec<&haze_chain::transaction::Input> {
...
pub(crate) fn all_outputs(block: &Block) -> Vec<&haze_chain::transaction::Output> {
...
pub async fn handle_fee_estimate(
    ...
    let estimate = FeeEstimate {
        ...
        fee_per_kb: haze_chain::mempool::FEE_PER_KB,
        mempool_size: mp.len(),
        suggested_name_fee: mp.suggested_name_fee(),
        min_name_fee: haze_chain::registry::NAME_REGISTRATION_FEE,
        name_ops_size: mp.name_ops_len(),
        suggested_asset_fee: mp.suggested_asset_fee(),
        min_asset_fee: haze_chain::assets::ASSET_MINT_FEE,
